...
import sys, os, platform, time, socket, signal
        ...
        for m in msgs:
            if mpstate.status.watch is not None:
                if fnmatch.fnmatch(m.get_type().upper(), mpstate.status.watch.upper()):
                    pass #mpstate.console.writeln('> '+ str(m))
...
if __name__ == '__main__':
    ...
    parser.add_option("--default-modules", default="", help='default module list')
