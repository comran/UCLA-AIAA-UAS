...
class Sony_Remote_Camera_Implementation : public Sony_Remote_Camera_Interface {
 public:
  ...
  virtual Sony_Capture_Error Get_Last_JPeg_Image(uint8_t *&data, size_t &size,
                                                 int &frame_number,
                                                 int &timestamp);
 ...
 private:
  ...
  void Handle_Write_HTTP_Request(bool mode_liveview, bool event_thread,
                                 const boost::system::error_code &err);
  void Handle_Read_Status_Line(bool mode_liveview, bool event_thread,
                               const boost::system::error_code &err,
                               const size_t bytes_transferred);
  void Handle_Read_Headers(bool mode_liveview, bool event_thread,
                           const boost::system::error_code &err);
  void Handle_Read_Text_Content(bool event_thread,
                                const boost::system::error_code &err);
  void Handle_Read_Common_Header(const boost::system::error_code &err,
                                 size_t payload_bytes_already_read,
  ...
  Sony_Capture_Error Send_Options_Command(std::string json_request,
                                          bool event_thread = false);
  std::string Build_JSON_Command(const std::string &method,
                                 const std::vector<std::string> &params);
  bool Check_Event(std::istream &json_answer, std::string name,
  ...
  bool got_it;
  int current_json_id;
  static const std::map<Camera_State::Shoot_Mode, std::string>
      shoot_modes_availables;
