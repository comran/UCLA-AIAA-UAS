...
const asio::ip::address SSDP_Client::multicast_address =
    address_v4::from_string("239.255.255.250");
...
SSDP_Client::SSDP_Client(asio::io_service &io_service, string my_own_ip)
  ...
  socket.async_send_to(buffer(multicast_search_request), endpoint,
                       boost::bind(&SSDP_Client::Handle_Send_Discovery_Request,
                                   this, boost::asio::placeholders::error));
};

void SSDP_Client::Handle_Send_Discovery_Request(
...
void SSDP_Client::Handle_Discovery_Timeout(
    const boost::system::error_code &error) {
  if (!error) {
    socket.close();
    ssdp_failure e(
        "No response from the device: wrong ip or no device listening on this "
        "network\n");
    throw(e);
  } else {
    cout << error;
...
void SSDP_Client::Handle_Read_Header(const boost::system::error_code &err,
                                     size_t bytes_recvd) {
  if (!err) {
    // Check that response is OK.
    stringstream http_response(data_);
    string http_version;
